...
| `generate-pdf.py` | WeasyPrint: HTML to PDF |
| `article-digest.md` | Compact proof points from portfolio (optional) |
| `interview-prep/story-bank.md` | Accumulated STAR+R stories across evaluations |
| `interview-prep/reframe-bank.md` | Career transition reframes and objection responses |
| `interview-prep/creative-audit-template.md` | Reusable Creative Audit template for interviews |
| `inbox/` | Drop zone for materials to review (CVs, JDs, PDFs, screenshots) |
...
| Wants interview prep | `interview` |
...
**NEVER trust WebSearch/WebFetch to verify if an offer is still active.** ALWAYS use the /browser skill:
1. Navigate to the URL
2. Read the page content
3. Only footer/navbar without JD = closed. Title + description + Apply = active.

**Exception for batch workers (`claude -p`):** The /browser skill is not available in headless pipe mode. Use WebFetch as fallback and mark the report header with `**Verification:** unconfirmed (batch mode)`. The user can verify manually later.

---

## Stack and Conventions

- Node.js (mjs modules), WeasyPrint/Python (PDF), /browser skill (web scraping), YAML (config), HTML/CSS (template), Markdown (data), Canva MCP (optional visual CV)

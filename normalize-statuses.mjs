...
function normalizeStatus(raw) {
  ...
  // Evaluada -> Evaluated (Spanish alias)
  if (/^evaluada$/i.test(s)) return { status: 'Evaluated' };

  // Entrevista -> Interview (Spanish alias)
  if (/^entrevista$/i.test(s)) return { status: 'Interview' };

  // Respondido -> Responded (Spanish alias)
  if (/^respondido$/i.test(s)) return { status: 'Responded' };

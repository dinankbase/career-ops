...
Processes job posting URLs accumulated in `data/pipeline.md`. The user adds URLs at any time and then runs `/career-ops pipeline` to process them all.
...
2. **For each pending URL**:
   ...
   b. **Extract JD** using the `/browser` skill → WebFetch → WebSearch
...
1. **`/browser` skill (preferred):** Use the `/browser` skill to navigate to the URL and read the page content. Works with all SPAs.
2. **WebFetch (fallback):** For static pages or when the `/browser` skill is not available.

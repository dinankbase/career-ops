...
Interactive mode for when the candidate is filling out an application form in Chrome. Reads what is on screen, loads the prior context for the posting, and generates personalized answers for each form question.

## Requirements

- **Best with the `/browser` skill**: The `/browser` skill lets Claude interact with the page while the candidate watches.
- **Without `/browser` skill**: The candidate shares a screenshot or pastes the questions manually.
...
5. COMPARE    → Does the role on screen match the one evaluated? If it changed → alert
...
**With `/browser` skill:** Use the `/browser` skill to take a snapshot of the active page. Read title, URL, and visible content.

**Without `/browser` skill:** Ask the candidate to:
...
4. If there is a Section G → load the previous draft answers as a base
5. If there is NO match → alert and offer to run a quick auto-pipeline

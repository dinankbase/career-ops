...
### Level 1 -- Direct browser navigation (PRIMARY)

**For each company in `tracked_companies`:** Navigate to its `careers_url` using the `/browser` skill, read ALL visible job listings, and extract title + URL from each one. This is the most reliable method because:
...
For companies using Greenhouse, the JSON API (`boards-api.greenhouse.io/v1/boards/{slug}/jobs`) returns clean structured data. Use as a quick complement to Level 1 - it's faster than browser navigation but only works with Greenhouse.
...
**Execution priority:**
1. Level 1: Browser → all `tracked_companies` with `careers_url`
...
4. **Level 1 -- Browser scan** (parallel in batches of 3-5):
   For each company in `tracked_companies` with `enabled: true` and `careers_url` defined:
   a. Use the `/browser` skill to navigate to the `careers_url`
   b. Read the page content to find all job listings
   c. If the page has filters/departments, navigate to the relevant sections
...
8.5. **Verify liveness of WebSearch results (Level 3)** -- BEFORE adding to pipeline:
   ...
   WebSearch results can be stale (Google caches results for weeks or months). To avoid evaluating expired offers, verify each new URL from Level 3 using the `/browser` skill. Levels 1 and 2 are inherently real-time and do not require this verification.

   For each new Level 3 URL (sequential - NEVER run browser automation in parallel):
   a. Use the `/browser` skill to navigate to the URL
   b. Read the page content
   c. Classify:
   ...
   **Do not interrupt the entire scan if a URL fails.** If navigation gives an error (timeout, 403, etc.), mark as `skipped_expired` and continue with the next one.
...
Each company in `tracked_companies` should have a `careers_url` - the direct URL to their job listings page. This avoids looking it up every time.
...
3. Use the `/browser` skill to navigate and confirm it works

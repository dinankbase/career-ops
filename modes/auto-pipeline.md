...
1. **`/browser` skill (preferred):** Most job portals (Lever, Ashby, Greenhouse, Workday) are SPAs. Use the `/browser` skill to navigate to the URL and read the JD content.
...
Run exactly the same as `offer` mode (read `modes/offer.md` for all blocks A-F).
...
1. **Extract form questions**: Use the `/browser` skill to navigate to the form and take a snapshot. If questions cannot be extracted, use the generic questions.
2. **Generate answers** following the tone guidelines (see below).
3. **Save in the report** as section `## G) Draft Application Answers`.

### Generic questions (use if questions cannot be extracted from the form)
...
Register in `data/applications.md` with all columns including Report and PDF marked as ✅.

...
4. **For each pending URL**:
   a. Chrome: click on the job → read JD text from DOM
...
- `--retry-failed` — only retry failed items
...
- If it dies → re-run → reads `batch-state.tsv` → skips completed items
- Lock file (`batch-runner.pid`) prevents double execution
- Each worker is independent: failure on job #47 does not affect the rest
...
| Conductor dies | Re-run → reads state → skips completed items |

...
This file is a regression fixture for the text normalization pass in `generate-pdf.py` (issue #1). It contains the Unicode artifacts that can cause parsing errors or display issues in ATS systems and legacy parsers. Use it to verify that the normalizer converts them to ASCII-safe equivalents.
...
```bash
# From the project root, verify generate-pdf.py syntax:
python -c "import ast; ast.parse(open('generate-pdf.py').read())"

# Quick smoke test (no browser needed):
python -c "from generate_pdf import normalize_text_for_ats; print('OK')" 2>/dev/null || true
```

For an end-to-end test, generate a CV PDF from a known dirty HTML file and inspect the output:

```bash
python generate-pdf.py /tmp/dirty-cv.html /tmp/clean-cv.pdf --format=a4
# Expected log line:
